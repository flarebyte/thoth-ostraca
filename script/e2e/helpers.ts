...
export function runThoth(bin: string, args: string[], root: string): SpawnSyncReturns<string> {
...
export function saveOutputs(root: string, base: string, run: SpawnSyncReturns<string>): void {
  const tempDir = path.join(root, "temp");
  fs.mkdirSync(tempDir, { recursive: true });
  fs.writeFileSync(path.join(tempDir, `${base}.out.txt`), run.stdout);
  fs.writeFileSync(path.join(tempDir, `${base}.err.txt`), (run as any).stderr ?? "");
}
