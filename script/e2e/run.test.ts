...
test("thoth run executes discovery and respects gitignore by default", () => {
  ...
  const expectedOut = expectedJSONFromGolden(root, "testdata/run/discovery1_out.golden.json");
  const run = runThoth(bin, ["run", "--config", cfg], root);
  expect(run.status).toBe(0);
  expect(run.stderr).toBe("");
...
test("thoth run parses YAML records into {locator,meta}", () => {
  ...
  const expectedOut = expectedJSONFromGolden(root, "testdata/run/yaml1_out.golden.json");
  const run = runThoth(bin, ["run", "--config", cfg], root);
  expect(run.status).toBe(0);
  expect(run.stderr).toBe("");
  expect(run.stdout).toBe(expectedOut);
...
test("thoth run filters records via Lua predicate", () => {
  ...
  const expectedOut = expectedJSONFromGolden(root, "testdata/run/filter1_out.golden.json");
  const run = runThoth(bin, ["run", "--config", cfg], root);
  expect(run.status).toBe(0);
  expect(run.stderr).toBe("");
  expect(run.stdout).toBe(expectedOut);
...
test("thoth run maps records via Lua transform", () => {
  ...
  const expectedOut = expectedJSONFromGolden(root, "testdata/run/map1_out.golden.json");
  const run = runThoth(bin, ["run", "--config", cfg], root);
  expect(run.status).toBe(0);
  expect(run.stderr).toBe("");
  expect(run.stdout).toBe(expectedOut);
...
test("thoth run executes shell and postmap", () => {
  ...
  const expectedOut = expectedJSONFromGolden(root, "testdata/run/shell1_out.golden.json");
  const run = runThoth(bin, ["run", "--config", cfg], root);
  expect(run.status).toBe(0);
  expect(run.stderr).toBe("");
  expect(run.stdout).toBe(expectedOut);
...
test("thoth run reduces to count and prints full envelope", () => {
  ...
  const expectedOut = expectedJSONFromGolden(root, "testdata/run/reduce1_out.golden.json");
  const run = runThoth(bin, ["run", "--config", cfg], root);
  expect(run.status).toBe(0);
  expect(run.stderr).toBe("");
  expect(run.stdout).toBe(expectedOut);
...
test("keep-going with embedErrors=true embeds record errors and lists envelope errors", () => {
  ...
  const expectedOut = expectedJSONFromGolden(root, "testdata/run/keep1_embed_true_out.golden.json");
  const run = runThoth(bin, ["run", "--config", cfg], root);
  expect(run.status).toBe(0);
  expect(run.stderr).toBe("");
  expect(run.stdout).toBe(expectedOut);
...
test("keep-going with embedErrors=false only lists envelope errors", () => {
  ...
  const expectedOut = expectedJSONFromGolden(root, "testdata/run/keep1_embed_false_out.golden.json");
  const run = runThoth(bin, ["run", "--config", cfg], root);
  expect(run.status).toBe(0);
  expect(run.stderr).toBe("");
  expect(run.stdout).toBe(expectedOut);
